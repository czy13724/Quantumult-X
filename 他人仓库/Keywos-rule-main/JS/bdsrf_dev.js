...
if (/imrobot\/v1\/pub\/relation\/get_robot_list/.test(url)) {
  okk.data.robot_info.forEach((robot) => {
    robot.friend_bg_img = `https://raw.githubusercontent.com/Keywos/rule/main/mocks/bd/robot${
      Math.floor(Math.random() * 20) + 1
    }.jpg`;
  });
} else if (/v5\/custom_page\/layout/.test(url)) {
  if (
    okk.data.page_layout_content &&
    Array.isArray(okk.data.page_layout_content)
  ) {
  ...
  if (okk.data.page_data_content && Array.isArray(okk.data.page_data_content)) {
    okk.data.page_data_content = okk.data.page_data_content.filter(
      ({ module_desc }) =>
        keyi.includes(module_desc) &&
        !keys.some((keyword) => module_desc.includes(keyword))
    );
  }
} else if (/v5\/custom_page\/getdata/.test(url)) {

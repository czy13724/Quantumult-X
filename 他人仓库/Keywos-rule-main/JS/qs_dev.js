...
if(body == null){if(isSurgeiOS || isLanceX || isStashiOS || isEgern){
 ...
 $done({ response: { status: 404 ,body:{} } });}else{
    console.log("QX转换：未获取到body的链接为" + $request.url)
    $notification.post("QX转换：未获取到body","请检查网络及节点是否畅通","认为是bug?点击通知反馈","https://t.me/zhangpeifu")
...
//(response|request)-body
				...
				let reBdArg2 = x.split(" " + reBdType + "-body ")[2];
					...
					z[y - 1]?.match(/^#/) && script.push(z[y - 1]);
					script.push(
							`${noteK}replaceBody_${y} = type=http-${reBdType}, pattern=${reBdPtn}, requires-body=true, max-size=0, script-path=https://gitlab.com/lodepuly/vpn_tool/-/raw/main/Resource/Script/CommonScript/replace-body.js, timeout=60, argument="${reBdArg1}->${reBdArg2}"`);
					}else if (isStashiOS){
					z[y - 1]?.match(/^#/) && script.push("    " + z[y - 1]);

...
QuantumultX重写引用地址：
https://raw.githubusercontent.com/Keywos/rule/main/JS/TestFlightAccount.js

注：以上引用地址需要打开并使用KOP-XIAO资源解析器，如没有解析器请使用脚本配置：

[rewrite_local]
^https:\/\/testflight\.apple\.com\/v\d\/(app|account|invite)s\/ url script-analyze-echo-response https://raw.githubusercontent.com/Keywos/rule/main/JS/TestFlightAccount.js
...
async function runs() {
    ...
    if ((TFPush === 'false') && (/\/settings\/notifications\//.test(req.url))) {
        if (req.body) {
            req.body = JSON.parse(req.body);
            const pushTfTv = req.body.platformUpdates.appletvos;
            if ((pushTfTv.emailEnabled = true) || (pushTfTv.pushEnabled = true)){
                $.info(req.body);
                pushTfTv.pushEnabled = false;
                pushTfTv.emailEnabled = false;
                $.notify('TvOS','TF通知已关闭','');
                $.info(req.body);

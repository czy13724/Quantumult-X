/* cname - cache name 
 * Update: 2023.05.18 必须安装以下模块，关闭官方版本才能使用: 目前SubStore还未更新脚本持久化缓存超时
...
async function operatorss(proxies) {
  ...
  if (typeof scriptResourceCache === 'undefined') {
    ...
    if (target=="Surge"){
      $notification.post("Sub-Store未更新", "", "请点击或查看Log查看脚本说明安装对应版本", {url: "https://github.com/Keywos/rule/raw/main/name.js"})
    } else if (target=="Loon")
    ...
    await Promise.all(
      batch.map(async (proxy) => {
        try {
            ...
            if (flag) { 
            proxy.name = getflag(outip.countryCode) +FGF+ outip.country; 
              } else {
            proxy.name = outip.country;
            }
          proxy.qc =  proxy.server+"|"+outip.query;
        } catch (err) {}
      })
    );
    // await sleep(10);
  ...
  const Push = (PRSO == PRS) ? "\n无复用节点, " : "\n去除无效节点后剩" + PRSO + "个, ";
  $notification.post(`NAME: 共${PRS}个节点`,"",`${writelog}${readlog}${Push}耗时:${mTIme(timeDiff)}`)
  return proxies;
}
let APIREADKEY = 0;
let APIWRITEKEY = 0;
...
async function IPAPI(proxy) {
  ...
  if (cached) {
    ...
    let url;
    if (yw) {
        url = 'http://ip-api.com/json?&fields=status,message,country,countryCode,city,query,regionName';
        } else {
        url = 'http://ip-api.com/json?lang=zh-CN&fields=status,message,country,countryCode,city,query,regionName';
        }

function PLISTs(opts) {
	return new (class {
		constructor(opts = []) {
			this.name = "Plist v1.0.0";
			this.opts = opts;
			this.request = {
				"url": "https://json2plist-production.up.railway.app/convert.php",
				"headers": {
					"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
					"Accept": "text/javascript, text/html, application/xml, text/xml, */*",
				}
			}
		};

		parse(xml) {
			this.request.body = "do=plist2json&content=" + encodeURIComponent(xml);
			let json = await $.http.post(this.request).then(v => JSON.parse(v.body));
			$.log(`🚧 ${$.name}, PLISTs`, `json: ${JSON.stringify(json)}`, "");
			return json;
		};

		stringify(json = this.json) {
			this.request.body = "do=json2plist&content=" + encodeURIComponent(JSON.stringify(json));
			let xml = await $.http.post(this.request).then(v => v.body);
			$.log(`🚧 ${$.name}, PLISTs`, `json: ${JSON.stringify(json)}`, "");
			return xml;
		};
	})(opts)
}



























// Adding a dummy sgmodule commit(13)
// Adding a dummy plugin commit(11)
// Adding a dummy stoverride commit(8)

...
async function runs() {
  ...
  if (/accounts\/[a-z0-9-]{36}\/apps$/.test(req.url)) {
    ...
    const out = arr.filter(
      (r) => !r.previouslyTested && !obj.has(r.appAdamId) && obj.set(r.appAdamId, 1)
    );
    ...
    if (out.length) {
      ...
      $.write(
        out.reduce((l, v) => ((l[v.appAdamId] = v.aid), l), {}),
        "AppList"
      );
    }
...
function QueryFallback(o) {
  return $.http[req.method.toLowerCase()](ChangeHeaders(o))
    .then((r) => {
      ...
      if (/\/apps\/\d+\/builds\/\d+$/.test(req.url) && r.body) {
        //beta app page
        r.body = JSON.parse(r.body);
        r.body.data.builds.map(
          (e) =>
            (e.eula = `https://testflight.apple.com/v1/apps/${e.appAdamId}/builds/${e.id}/eula`)
        );
...
function QueryAppList(o) {
  return $.http[req.method.toLowerCase()](ChangeHeaders(o))
    .then((r) => {
      ...
      $.log(
        `Received response: status=${r.statusCode}, body=${Boolean(
          r.body
        )}, account=${o}, main=${m}`
      );
      if (m) {

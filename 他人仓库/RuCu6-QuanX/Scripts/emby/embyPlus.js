...
if (requestURL.indexOf(addLink) != -1) {
  // 添加外部播放器链接
    ...
    obj.MediaSources.forEach((item, index) => {
        ...
        if (t["Type"] === "Subtitle" && t["IsExternal"] && t["Path"]) {
          ...
          shuInfo.push({
            ...
            url:
              ...
              t["Codec"] +
              "/" +
              encodeURIComponent(subtitleFileName) +
              "?api_key=" +
              query["X-Emby-Token"] +
              "&filename=" +
              encodeURIComponent(subtitleFileName),
      ...
      shuDownload.push({
        Url:
          host +
          embyPlguin +
          "shu://gui.download.http?urls=" +
          encodeURIComponent(JSON.stringify(shuInfo)),
        Name: "Shu" + Name
      });
    });
  }
  ...
  if (obj.ExternalUrls) {
    // 兼容终点站的原生 url schema 连接
    obj.ExternalUrls = obj.ExternalUrls.filter(function (item) {
      return !(
        item.Name.indexOf("Infuse") != -1 ||
        item.Name.indexOf("nPlayer") != -1 ||
        item.Name.indexOf("VLC") != -1 ||
        item.Name.indexOf("IINA") != -1 ||
        item.Name.indexOf("Movist Pro") != -1
      );
    });
  }
  ...
  requestURL.indexOf("/Videos/") != -1 &&
  (requestURL.indexOf("/stream/") != -1 || requestURL.indexOf("/Subtitles/") != -1)
) {
  // 资源路径伪静态
  let query = getQueryVariable(requestURL);

...
if ($request.url.indexOf("/Download") != -1) {
  if ($util.status == 401) {
    ...
    let subtitle_stream_index =
      typeof query.subtitleStreamIndex != "undefined" ? query.subtitleStreamIndex : "";
    let type = query.type;
    ...
    let X_Emby_Authorization =
      'MediaBrowser Device="Download", DeviceId="' +
      X_Emby_Device_Id +
      '", Version="' +
      X_Emby_Client_Version +
      '", Client="' +
      X_Emby_Client +
      '", Token="' +
      api_key +
            ...
            if (media_source.Id == media_source_id) {
              ...
              switch (type) {
                case "shu_download":
                  ...
                  console.log(
                    "《" + video_data.SortName + "》 Shu 批量下载地址:\n" + shu_download_url + "\n"
                  );
                  //$util.done({status: 301, headers: {Location:shu_download_url} });
                  $util.done({
                    body: "Shu download",
                    headers: { Location: shu_download_url },
                ...
                case "nplayer_play":
                  ...
                  $util.done({
                    body: "nPlayer play",
                    headers: { Location: nplayer_url_scheme },
                    status: "HTTP/1.1 302 Found"
                  });
                  break;
                case "vlc_play":
                  ...
                  console.log(
                    "《" + video_data.SortName + "》 VLC 播放地址:\n" + vlc_url_scheme + "\n"
                  );
                  $util.done({
                    body: "VLC play",
                    headers: { Location: vlc_url_scheme },
                    status: "HTTP/1.1 302 Found"
                  });
                  break;
                case "infuse_play":
                  ...
                  console.log(
                    "《" + video_data.SortName + "》 Infuse 播放地址:\n" + infuse_url_scheme + "\n"
                  );
                  $util.done({
                    body: "Infuse play",
                    headers: { Location: infuse_url_scheme },
                    status: "HTTP/1.1 302 Found"
                  });
                  break;
                default:
                  if (subtitle_stream_index !== "" && subtitle_stream_index != -1) {
                    ...
                    for (let key in download_info.subtitles) {
                      if (download_info.subtitles[key].index == subtitle_stream_index) {
                        subtitle_download_url =
                          download_info.subtitles[key].url +
                          "&filename=" +
                          encodeURI(download_info.subtitles[key].filename);
                        break;
                      }
                    }
                    if (subtitle_download_url === "") {
                      ...
                      $util.done({
                        body: "video download",
                        headers: { Location: download_info.video.original_url },
                        status: "HTTP/1.1 302 Found"
                      });
                    } else {
                      console.log(
                      ...
                      $util.done({
                        body: "subtitle download",
                        headers: { Location: subtitle_download_url },
                        status: "HTTP/1.1 302 Found"
                      });
                    }
                  } else {
...
if ($request.url.indexOf("/web/item/item.js") != -1) {
  ...
  let replace =
    'var selectSource=view.querySelector(".selectSource"),selectSubtitles=view.querySelector(".selectSubtitles"),';
  body = body.replace(find, replace);
  find = "mediaSourceId:selectSource&&selectSource.value||null,";
  replace =
    'mediaSourceId:selectSource&&selectSource.value||null,subtitleStreamIndex:selectSubtitles&&selectSubtitles.value||"",';
  body = body.replace(find, replace);
...
if ($request.url.indexOf("/web/modules/itemcontextmenu.js") != -1) {
  let body = $response.body;
  let find =
    ',"MediaStream"===item.Type&&item.IsExternal&&"Subtitle"===item.StreamType&&appHost.supports("filedownload")';
  let replace =
    '&&commands.push({name:"\u0053\u0068\u0075\u0020\u4e0b\u8f7d",id:"shu_download",icon:"download"})&&commands.push({name:"\u006e\u0050\u006c\u0061\u0079\u0065\u0072\u0020\u64ad\u653e",id:"nplayer_play",icon:"play_arrow"})&&commands.push({name:"\u0056\u004c\u0043\u0020\u64ad\u653e",id:"vlc_play",icon:"play_arrow"})&&commands.push({name:"\u0049\u006e\u0066\u0075\u0073\u0065\u0020\u64ad\u653e",id:"infuse_play",icon:"play_arrow"}),"MediaStream"===item.Type&&item.IsExternal&&"Subtitle"===item.StreamType&&appHost.supports("filedownload")';
  body = body.replace(find, replace);
...
function downloadInfo(host, video_id, media_source, api_key) {
  ...
  for (let key in media_source.MediaStreams) {
    ...
    if (
      media_streams.Type == "Subtitle" &&
      media_streams.IsExternal == 1 &&
      media_streams.IsTextSubtitleStream == 1
    ) {
...
function generateCURL(data) {
  ...
  command +=
    '-o "' +
    data.video.filename.replace(/"/g, '"') +
    '" ' +
    '"' +
    data.video.original_url.replace(/"/g, '"') +
    '" ';
  for (let key in data.subtitles) {
    command +=
      '-o "' +
      data.subtitles[key].filename.replace(/"/g, '"') +
      '" ' +
      '"' +
      data.subtitles[key].url +
      '" ';
  }
  return command;
}
...
function generateVlcURLScheme(data, subtitle_stream_index) {
  let vlc_x_callback =
    "vlc-x-callback://x-callback-url/stream?url=" + encodeURIComponent(data.video.url);
  for (let key in data.subtitles) {

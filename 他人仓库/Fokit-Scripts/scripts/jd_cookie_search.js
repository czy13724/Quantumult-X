...
;(async () => {
  ...
  for (const cookie of ckData) {
    ...
    if (response.retcode === '0') {
      ...
      // if (!mobile) mobile = await getPhoneNumber(cookie)  // 手机号码
      // fruit = await getFruit(cookie)  // 农场进度
      // jdPet = await PetRequest('energyCollect', cookie)  // 萌宠进度
    }

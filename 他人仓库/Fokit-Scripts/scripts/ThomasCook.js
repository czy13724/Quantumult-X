...
// 请求函数二次封装
function httpRequest(options, method = 'get') { if ('body' in options) { method = 'post' }; return new Promise((resolve) => { $[method](options, (err, resp, data) => { try { if (err) { console.log(`❌ ${options['url']}请求失败`); $.logErr(err); } else { if (data) { typeof JSON.parse(data) == 'object' ? (data = JSON.parse(data)) : ''; resolve(data); } else { console.log(`服务器返回空数据`); } } } catch (e) { $.logErr(e, resp); } finally { resolve(); } }) }) }
// prettier-ignore

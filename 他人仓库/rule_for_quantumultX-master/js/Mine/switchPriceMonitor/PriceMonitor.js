...
const keyName = 'SwitchPriceMonitor'
const $ = new Env(title, true)
let Monitor = {
    list: [{name:'塞尔狂野之息dlc',id:'70070000000024'},{name: '怪物猎人崛起本体',id:'70010000029014'}],
    isNotify: true
}
...
async function CheckPrice(item){
    ...
    await $.http.get({url:url}).then(response=>{
        ...
        obj.prices.forEach(function (i) {
            ...
            if(i.discount_price)
            {
                const regular_price = i.regular_price.raw_value // 日常价格
                const discount_price = i.discount_price.raw_value // 折扣价格

...
locateInfo = $.getjson(keyName) || locate
const now = new Date()
let year = String(now.getFullYear())
let month = String(now.getMonth() + 1)
let day = String(now.getDate())
let host = 'https://raw.githubusercontent.com/zqzess/openApiData/main/gasolinePrice/'
if (month.length === 1)
    month = '0' + month
if (day.length === 1)
    day = '0' + day
let option = {
    url: host + year + '/' + month + '/' + year + month + day + '.json',
    headers: {}
}
console.log('链接: ' + option.url)
console.log('地区:' + locateInfo.location)
console.log('油号: ' + locateInfo.type)
locateInfo.type = locateInfo.type.replace('号', '')
let notifyContent = ''
let body = {}
$.get(option, function (error, response, data) {
    ...
    $.isSurge() ? body = {
        ...
        icon: 'fuelpump',
        'icon-color': '#FFCD00'
    } : body = {title: title, content: notifyContent + '\n' + jsonObj.message, icon: 'oilcan', backgroundColor: '#FFCD00'}
    console.log('\n内容：\n' + notifyContent)

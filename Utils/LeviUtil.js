...
async function loadLeviUtils() {
    ...
    if (code && Object.keys(code).length) {
        console.log(`✅ ${$.name}: 缓存中存在LeviUtil代码, 跳过下载`)
        eval(code)
        return creatUtils();
    }
    console.log(`🚀 ${$.name}: 开始下载LeviUtil代码`)
    return new Promise(async (resolve) => {
        $.getScript(
            'https://cdn.jsdelivr.net/gh/czy13724/Quantumult-X@main/Utils/LeviUtil.js'
        ).then((fn) => {
            $.setdata(fn, LeviUtil_code)
            eval(fn)
            const LeviUtil = creatUtils();
            console.log(`✅ LeviUtil加载成功, 请继续`)
            resolve(LeviUtil)
        })
    })
}
